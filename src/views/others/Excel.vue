  ...
  <div style="width: 100%">
    <div class="option-area">
      ...
      <div class="button-area">
        <a-button type="primary" @click="downloadTemplate" style="margin-right: .5rem">模板下载</a-button>
        <a-button @click="exportExcel" style="margin-right: .5rem">导出Excel</a-button>
        <a-button
          @click="handleUpload"
          :disabled="fileList.length === 0"
          :loading="uploading"
        >{{uploading ? '导入中' : '导入Excel' }}</a-button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleUpload() {
      ...
      this.uploading = true
      this.$upload('test/import', formData)
        .then(r => {
