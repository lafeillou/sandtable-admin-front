        ...
        <a-col :span="span" v-for="(movie, index) in movies" :key="index" style="padding: 1rem;">
          <a-card class="movie-card" style="width: 200px" @click="detail(movie.id)">
            ...
            <div class="movie-desc">
              <div class="movie-title" :title="movie.tCn">
                <p>{{movie.tCn}}</p>
              </div>
              ...
              <span>
                <a-row :gutter="2">
                  ...
                  <a-col :span="12">
                    ...
                    <p
                      v-if="movieDetail.boxOffice.todayBoxDesUnit"
                    >{{movieDetail.boxOffice.todayBoxDesUnit}}：{{movieDetail.boxOffice.todayBoxDes}}</p>
                    <p
                      v-if="movieDetail.boxOffice.totalBoxUnit"
                    >{{movieDetail.boxOffice.totalBoxUnit}}：{{movieDetail.boxOffice.totalBoxDes}}</p>
                    <p>
                      预告片：
                      <a @click="video(movieDetail.basic.video.hightUrl)">点击查看</a>
                    </p>
                    <a-popover :title="movieDetail.basic.director.name">
                      <template slot="content">
                        <img
                          alt="头像"
                          :src="movieDetail.basic.director.img"
                          style="width:140px; height: 180px"
                        />
                      </template>
                      <p>
                        导演：
                        <a>{{movieDetail.basic.director.name}}</a>
                      </p>
                    </a-popover>
                        ...
                        <a-popover :key="index" :title="actor.name">
                          <template slot="content">
                            <div>
                              ...
                              >扮演：{{actor.roleName ? actor.roleName : '未知 X_X'}}</p>
                              <img alt="头像" :src="actor.img" style="width:140px; height: 180px" />
                            </div>
                          </template>
                          <span v-if="actor.name">
                            <span v-if="index === 0">
                              <a style="margin-left: -6px;">{{actor.name}}</a> ·
                            </span>
                            <span v-else-if="index === movieDetail.basic.actors.length - 1">
                              <a>{{actor.name}}</a>
                            </span>
                            <span v-else>
                              <a>{{actor.name}}</a> ·
                ...
                <a-list-item slot="renderItem" slot-scope="comment">
                  <a-list-item-meta :description="comment.content">
                    <div slot="title">
                      {{comment.nickname}}&nbsp;
                      <span
                        style="color: #aaa;font-weight: 400; font-size: 13px"
                      >{{getDate(comment.commentDate)}}</span>
                    </div>
