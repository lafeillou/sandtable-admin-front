<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        ...
        <a-upload
          ...
          :headers="headers"
          :action="baseURL + '/api/v1/sandtable_target/batch_insert'"
          accept=".xls, .xlsx"
      ...
      >
        <template slot="operation" slot-scope="text, record">
          ...
          ></a-icon>
          <a-icon
            v-hasPermission="['target:soldiers']"
            type="team"
            title="兵员"
            style="margin-left: 6px;color:#42b983"
            @click="setTeam(record)"
          />
          <a-badge v-hasNoPermission="['target:update']" status="warning" text="无权限"></a-badge>
        </template>
      </a-table>
    </div>
    <target-add @success="handleAddSuccess" @close="handleAddClose" :addVisiable="addVisiable"></target-add>
    <target-edit
      ref="editDialog"
    ...
    <set-team
      ref="setTeamDialog"
      :visible="setTeamVisible"
      @success="handleSetTeamSuccess"
      @close="handleSetTeamClose"
    ></set-team>
  </a-card>
</template>
...
export default {
  name: 'SandtableTarget',
  components: { TargetAdd, TargetEdit, SetTeam, RangeDate },
  data() {
    return {
      baseURL,
      advanced: false,
      ...
      selectedRowKeys: [],
      queryParams: {
        queryAreaType: this.$store.state.setting.queryAreaType,
        queryAreaName: this.$store.state.setting.queryAreaName,
        keyword: '',
        pageNum: 1,
        pageSize: 10
      },
  ...
  methods: {
    ...
    // 兵员分配
    setTeam(record) {
      this.$refs.setTeamDialog.target = record
      this.setTeamVisible = true
    },
    handleSetTeamSuccess() {
      this.setTeamVisible = false
    },
    handleSetTeamClose() {
      this.setTeamVisible = false
    },
    ...
    batchDelete() {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          that.$delete('/api/v1/sandtable_target/' + ids.join(',')).then(() => {
    ...
    fetch(params) {
      ...
      this.loading = true
      this.$get('/api/v1/sandtable_target/listByArea', {
        ...params
      }).then(r => {
